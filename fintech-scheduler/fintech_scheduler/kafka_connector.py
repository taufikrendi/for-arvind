...
def on_delivery(err, msg, obj):
    if err is not None:
        logger.error('Message Scheduler Service {} delivery failed for user {} with error {}'.format(
            obj.email, obj.email, err))
    else:
        logger.info('Message Scheduler Service {} successfully produced to {} [{}] at offset {}'.format(
...
def commit_completed(err, partitions):
    if err:
        ...
        logger.info("Committed Scheduler Service partition offsets: " + str(partitions))
